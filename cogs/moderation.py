...
class moderation(commands.Cog):
  ...
  async def ban(self, ctx, member: discord.Member=None):
    ...
    embed.add_field(name="BOT BanHammer", value="****{}**** was banned" .format(member.name), inline=False)
    await ctx.send(embed=embed)
    await member.ban()
  ...
  async def embed(self, ctx, *, message):
    ...
    embed.set_author(name="BOT Embeder")
  ...
  async def warn(self, ctx, member, *, reason = "unspecified"):
    ...
    embed.add_field(name="BOT mod warner", value=f"{ctx.author.mention} warned {member}\n reason - {reason}")
